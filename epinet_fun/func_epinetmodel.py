...
def define_epinet(sz_input, sz_input2, view_n, conv_depth, filt_num, learning_rate):
    ...
    input_stack_90d = Input(
        shape=(sz_input, sz_input2, len(view_n)), name="input_stack_90d"
    )
    input_stack_0d = Input(
        shape=(sz_input, sz_input2, len(view_n)), name="input_stack_0d"
    )
    input_stack_45d = Input(
        shape=(sz_input, sz_input2, len(view_n)), name="input_stack_45d"
    )
    input_stack_M45d = Input(
        shape=(sz_input, sz_input2, len(view_n)), name="input_stack_M45d"
    )

    """ 4-Stream layer : Conv - Relu - Conv - BN - Relu """
    mid_90d = layer1_multistream(sz_input, sz_input2, len(view_n), int(filt_num))(
        input_stack_90d
    )
    mid_0d = layer1_multistream(sz_input, sz_input2, len(view_n), int(filt_num))(
        input_stack_0d
    )
    mid_45d = layer1_multistream(sz_input, sz_input2, len(view_n), int(filt_num))(
        input_stack_45d
    )
    mid_M45d = layer1_multistream(sz_input, sz_input2, len(view_n), int(filt_num))(
        input_stack_M45d
    )
    ...
    mid_merged_ = layer2_merged(
        sz_input - 6, sz_input2 - 6, int(4 * filt_num), int(4 * filt_num), conv_depth
    )(mid_merged)

    """ Last Conv layer : Conv - Relu - Conv """
    output = layer3_last(
        sz_input - 18, sz_input2 - 18, int(4 * filt_num), int(4 * filt_num)
    )(mid_merged_)
