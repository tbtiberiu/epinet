    ...
    for dir_LFimage in dir_LFimages:
        ...
        for i in range(81):
            try:
                tmp = np.float32(
                    imageio.imread(
                        "hci_dataset/" + dir_LFimage + "/input_Cam0%.2d.png" % i
                    )
                )  # load LF images(9x9)
            except:
                print(
        ...
        try:
            tmp = np.float32(
                read_pfm("hci_dataset/" + dir_LFimage + "/gt_disp_lowres.pfm")
            )  # load LF disparity map
        except:
            print(
                "hci_dataset/" + dir_LFimage + "/gt_disp_lowres.pfm..does not exist" % i
            )
