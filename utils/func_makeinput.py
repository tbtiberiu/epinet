    ...
    for seq in seq1:
        tmp = np.float32(
            imageio.imread(
                image_path
                + "/%s_%02d_%02d.png"
                % (image_path.split("/")[-1], 1 + seq // 9, 1 + seq - (seq // 9) * 9)
            )
        )
        traindata_tmp[0, :, :, i] = (
            RGB[0] * tmp[:, :, 0] + RGB[1] * tmp[:, :, 1] + RGB[2] * tmp[:, :, 2]
...
def make_multiinput(image_path, image_height, image_width, view_n):
    ...
    seq90d = list(
        range(4, 81, 9)[::-1][slice_for_5x5 : 9 - slice_for_5x5 :]
    )  # 90degree:  [76, 67, 58, 49, 40, 31, 22, 13, 4 ]
    seq0d = list(
        range(36, 45, 1)[slice_for_5x5 : 9 - slice_for_5x5 :]
    )  # 0degree:  [36, 37, 38, 39, 40, 41, 42, 43, 44]
    seq45d = list(
        range(8, 80, 8)[::-1][slice_for_5x5 : 9 - slice_for_5x5 :]
    )  # 45degree:  [72, 64, 56, 48, 40, 32, 24, 16, 8 ]
    seqM45d = list(
        range(0, 81, 10)[slice_for_5x5 : 9 - slice_for_5x5 :]
    )  # -45degree:  [0, 10, 20, 30, 40, 50, 60, 70, 80]

    if image_path[:16] == "data/hci_dataset":
